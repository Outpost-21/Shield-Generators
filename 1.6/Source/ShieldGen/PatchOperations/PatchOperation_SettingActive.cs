...
namespace ShieldGen
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = ShieldGenMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!ShieldGenMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (ShieldGenMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
